...
class FM(BasicModel):
    def __init__(self, opt):
        ...
        self.res_fm = FactorizationMachine(reduce_sum=True)
    ...
    def res_forward(self, x):
        ...
        res_output_fm = self.res_fm(res_x_embedding)
...
class DNN(BasicModel):
    def __init__(self, opt):
        ...
        self.res_dnn = MultiLayerPerceptron(self.dnn_dim, embed_dims, dropout, use_bn=use_bn)
    ...
    def res_forward(self, x):
        ...
        res_output_dnn = self.res_dnn(res_x_dnn)
...
class DeepCrossNet(BasicModel):
    def __init__(self, opt):
        ...
        self.res_cross = layer.CrossNetwork(self.dnn_dim, cross_num)
        self.res_dnn = MultiLayerPerceptron(self.dnn_dim, mlp_dims, output_layer=False, dropout=dropout, use_bn=use_bn)
        self.res_combination = torch.nn.Linear(mlp_dims[-1] + self.dnn_dim, 1, bias=False)
    ...
    def res_forward(self, x):
        res_x_embedding = torch.sigmoid(self.res_embedding(x))
        res_x_dnn = res_x_embedding.view(-1, self.dnn_dim)
        res_output_cross = self.res_cross(res_x_dnn)
        res_output_dnn = self.res_dnn(res_x_dnn)
        res_comb_tensor = torch.cat((res_output_cross, res_output_dnn), dim=1)
        res_logit = self.res_combination(res_comb_tensor)
        return res_logit
...
class InnerProductNet(BasicModel):
    def __init__(self, opt):
        ...
        self.res_inner = layer.InnerProduct(self.field_num)
        self.res_dnn = MultiLayerPerceptron(self.dnn_dim, mlp_dims, output_layer=True, dropout=dropout, use_bn=use_bn)

    def forward(self, x):
        x_embedding = self.embedding(x)
        x_dnn = x_embedding.view(-1, self.field_num * self.latent_dim)
        x_inner_product = self.inner(x_embedding)
        x_dnn = torch.cat((x_dnn, x_inner_product), 1)
        logit = self.dnn(x_dnn)
        return logit

    def res_forward(self, x):
        res_x_embedding = torch.sigmoid(self.res_embedding(x))
        res_x_dnn = res_x_embedding.view(-1, self.field_num * self.latent_dim)
        res_x_inner_product = self.res_inner(res_x_embedding)
        res_x_dnn = torch.cat((res_x_dnn, res_x_inner_product), 1)
        res_logit = self.res_dnn(res_x_dnn)
        return res_logit
