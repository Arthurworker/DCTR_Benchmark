...
def main(args):
    ...
    opt = {"model_opt": model_opt, "dataset": args.dataset, "model": args.model, "data_dir": data_dir,
           "save_dir": args.save_dir, "cuda": args.cuda, "source": args.source}
    print(opt)
    ...
    hyper_parameter_tuning(model_opt, params, dataset=args.dataset, model=args.model,
                           save_path=args.dataset+'/'+args.model+'/'+args.table_name, data_dir=data_dir,
                           save_dir=args.save_dir, source=args.source, pretrain=args.pretrain, seed=args.seed,
                           cuda=args.cuda)


if __name__ == "__main__":
    # Commandline arguments
    parser = argparse.ArgumentParser(description="CTR trainer")
    # 'coat' or 'kuairand'
    ...
    parser.add_argument("-save_dir", type=str, default="pretrain/", help="model save directory")

    # mode for code, pretrain/train
    parser.add_argument("-pretrain", type=int, help="pretrain", default=0)  # 0: train, 1: pretrain
