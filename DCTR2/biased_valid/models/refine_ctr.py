...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        print('beta1', beta1, 'lr:', lr, 'l2:', l2, 'bs:', batch_size)
        model = RefineCTR(self.model_opt, self.dataset, self.model, self.optimizer, self.data_dir, self.save_dir,
...
class RefineCTR(torch.nn.Module):
    def __init__(self, model_opt, dataset, model, optimizer, data_dir, save_dir, bsize,
        ...
        self.network.load_state_dict(torch.load('pretrain/unif_val/' + self.dataset + '_' + self.model + '_bias_featPlus2' + '.pt'))
        self.unif_network.load_state_dict(torch.load('pretrain/unif_val/' + self.dataset + '_' + self.model + '_unif_featPlus2' + '.pt'))
...
def refinectr(model_opt, dataset, model, data_dir, save_dir, seed=0, bsize=256, beta1=0.5, lr=5e-4, l2=1e-5,
              n_trials=100, epoch=5000, cuda=0, searcher='grid', optimizer='Adam', **unused):
    progress = WorkSplitter()

    progress.section("Refine-CTR (+2 feat): Set the random seed")
    setup_seed(seed)

    progress.section("Refine-CTR (+2 feat): Training")
    ...
    if searcher == 'grid':
        ...
        model = RefineCTR(model_opt, dataset, model, optimizer, data_dir, save_dir, bsize, beta1, lr, l2, cuda
                          ).to(device)
