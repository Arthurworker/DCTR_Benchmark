...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = IDIPSCTR(self.model_opt, self.dataset, self.model, self.optimizer, self.data_dir, self.save_dir,
...
def idipsctr(model_opt, dataset, model, data_dir, save_dir, seed=0, bsize=256, beta1=0.5, lr=5e-4, l2=1e-5,
             n_trials=100, epoch=5000, cuda=0, searcher='grid', optimizer='Adam', **unused):
    progress = WorkSplitter()

    progress.section("IDIPS-CTR (+2 feat): Set the random seed")
    setup_seed(seed)

    progress.section("IDIPS-CTR (+2 feat): Estimate pscore")
    ...
    progress.section("IDIPS-CTR (+2 feat): Training")
    ...
    if searcher == 'grid':
        ...
        model = IDIPSCTR(model_opt, dataset, model, optimizer, data_dir, save_dir, bsize, beta1, lr, l2, cuda
                         ).to(device)
