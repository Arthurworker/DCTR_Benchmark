...
class Objective:
    ...
    def __init__(self, model_opt, dataset, pscore, model, optimizer, data_dir, save_dir, seed, device, cuda,
        ...
        if dataset == 'kuairand':
            item_field_num = len(self.item_feat_dict[1])
        else:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = DRCTR(self.model_opt, self.dataset, self.model, self.optimizer, self.data_dir, self.save_dir,
...
class DRCTR(torch.nn.Module):
    def __init__(self, model_opt, dataset, model, optimizer, data_dir, save_dir, user_feat_tab, item_feat_tab,
                 all_user_id, all_item_id, bsize, beta1=0.5, lr=1e-3, l2=1e-2, cuda=0):
        super(DRCTR, self).__init__()
        self.lr = lr
        self.l2 = l2
        self.bs = bsize
        self.beta1 = beta1
        ...
        self.imputation_network.load_state_dict(torch.load(
            'pretrain/unif_val/' + self.dataset + '_' + self.model + '_combine_featPlus2' + '.pt'))

    def train_on_batch(self, label, data, sample_data, ips_weight):
        self.network.train()
        ...
        imp_label1 = torch.sigmoid(self.imputation_network(data))
        imp_label2 = torch.sigmoid(self.imputation_network(sample_data))

        ips_loss = torch.mean(ips_weight * self.criterion(
            self.network(data), label) - self.criterion(self.network(data), imp_label1))
        direct_loss = torch.mean(self.criterion(self.network(sample_data), imp_label2))
    ...
    def fit(self, epochs, pscore, early_stop_cnt):
        early_stop_step = 0
        best_auc = 0.0
        ...
        for _ in tqdm(range(int(epochs))):
            for feature, user_id, item_id, label, mark in train_data:
                ...
                _ = self.train_on_batch(label, feature, sample_data, ips_weight)
...
def drctr(model_opt, dataset, model, data_dir, save_dir, seed=0, bsize=256, beta1=0.5, lr=5e-4, l2=1e-5,
          n_trials=100, epoch=5000, cuda=0, searcher='grid', optimizer='Adam', **unused):
    progress = WorkSplitter()

    progress.section("DR-CTR (+2 feat): Set the random seed")
    setup_seed(seed)

    progress.section("DR-CTR (+2 feat): Estimate pscore")
    ...
    progress.section("DR-CTR (+2 feat): Training")
    ...
    if searcher == 'grid':
        ...
        model = DRCTR(model_opt, dataset, model, optimizer, data_dir, save_dir, bsize, beta1, lr, l2, epoch, cuda
                      ).to(device)
