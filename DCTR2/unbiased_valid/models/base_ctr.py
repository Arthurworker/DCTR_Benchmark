...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = BASECTR(self.model_opt, self.dataset, self.model, self.optimizer, self.data_dir, self.save_dir,
...
def basectr(model_opt, dataset, model, data_dir, save_dir, source=None, seed=0, bsize=256, lr=5e-4, l2=1e-5,
            n_trials=50, epoch=500, cuda=0, searcher='grid', optimizer='Adam', **unused):
    ...
    if searcher == 'grid':
        ...
        model = BASECTR(model_opt, dataset, model, optimizer, data_dir, save_dir, bsize, lr, l2, cuda, source
                        ).to(device)
