...
def feature_example(label, user_id, item_id, feature, mark):
    feature_des = {
        'feature': tf.train.Feature(int64_list=tf.train.Int64List(value=feature)),
        'user_id': tf.train.Feature(float_list=tf.train.FloatList(value=[user_id])),
        'item_id': tf.train.Feature(float_list=tf.train.FloatList(value=[item_id])),
        'label': tf.train.Feature(float_list=tf.train.FloatList(value=[label])),
        'mark': tf.train.Feature(float_list=tf.train.FloatList(value=[mark]))
    }
...
class DataTransform(object):
    ...
    def _read(self, name=None, header=None, sep=None, label_index="", id=[]):
        print("=====read data=====")

        self.traindata = pd.read_table(self.data_path1, names=name, header=header, sep=sep)
        self.testdata = pd.read_table(self.data_path2, names=name, header=header, sep=sep)
    ...
    def generate_and_filter(self, threshold=0, label_index="", id=[], white_list=[]):
        self.field_offset = {}
        offset = 0
        for field in self.field_name:
            # print(field)
    ...
    def transform_tfrecord(self, data, record_path, flag, records=5e6, label_index="", id=[], mark=1):
        ...
        while records * part <= data.shape[0]:
            ...
            for index, row in tmp_data.iterrows():
                ...
                # if oov:
                # continue
                tf_writer.write(feature_example(label, user_id, item_id, feature, mark))
                pbar.update(1)
                instance_num += 1
            tf_writer.close()
            pbar.close()
