...
class Objective:
    ...
    def __init__(self, model_opt, dataset, invP, model, optimizer, data_dir, seed, device, cuda,
                 epoch=500) -> None:
        """Initialize Class"""
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = IPSCTR(self.model_opt, self.dataset, self.model, self.optimizer, self.data_dir,
...
class Tuner:
    ...
    @staticmethod
    def tune(n_trials, model_opt, dataset, invP, model, optimizer, data_dir, seed, device, epoch, cuda):
...
class IPSCTR(torch.nn.Module):
    ...
    def fit(self, epochs, invP, early_stop_cnt, with_test=False):
        early_stop_step = 0
        best_auc = 0.0
...
def ipsctr(model_opt, dataset, model, data_dir, seed=0, bsize=256, lr=5e-4, l2=1e-5, n_trials=50, epoch=5000,
           cuda=0, searcher='grid', optimizer='Adam', with_test=False, **unused):
    progress = WorkSplitter()

    progress.section("IPS-CTR: Set the random seed")
    setup_seed(seed)

    progress.section("IPS-CTR: Estimate pscore")
    ...
    progress.section("IPS-CTR: Training")
    ...
    if searcher == 'grid':
        ...
        model = IPSCTR(model_opt, dataset, model, optimizer, data_dir, bsize, lr, l2, cuda).to(device)
