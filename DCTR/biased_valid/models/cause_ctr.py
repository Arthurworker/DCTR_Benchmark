...
class Objective:
    ...
    def __init__(self, model_opt, dataset, model, optimizer, data_dir, seed, device, cuda, user_feat_id, item_feat_id,
                 epoch=500) -> None:
        """Initialize Class"""
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = CausECTR(self.model_opt, self.dataset, self.model, self.optimizer, self.data_dir,
...
class Tuner:
    ...
    @staticmethod
    def tune(n_trials, model_opt, dataset, model, optimizer, data_dir, seed, device, epoch, cuda, user_feat_id,
             item_feat_id):
        ...
        objective = Objective(model_opt=model_opt, dataset=dataset, model=model, optimizer=optimizer, data_dir=data_dir,
                              seed=seed, epoch=epoch, device=device, cuda=cuda, user_feat_id=user_feat_id,
                              item_feat_id=item_feat_id)
...
class CausECTR(torch.nn.Module):
    def __init__(self, model_opt, dataset, model, optimizer, data_dir, user_feat_id, item_feat_id, bsize,
        ...
        self.model_opt['feat_num'] = self.model_opt['feat_num'] + self.item_feat_id['max'] - self.item_feat_id[
            'min'] + 1
    ...
    def fit(self, epochs, early_stop_cnt, with_test=False):
        early_stop_step = 0
        best_auc = 0.0
...
def causectr(model_opt, dataset, model, data_dir, seed=0, bsize=256, alpha1=0.5, lr=5e-4, l2=1e-5, n_trials=100,
    ...
    user_feat_id = {'max': 0, 'min': float('inf')}
    item_feat_id = {'max': 0, 'min': float('inf')}

    for k, v in user_feat_dict.items():
        if max(v) > user_feat_id['max']:
            user_feat_id['max'] = max(v)
        if min(v) < user_feat_id['min']:
            user_feat_id['min'] = min(v)
    for k, v in item_feat_dict.items():
        if max(v) > item_feat_id['max']:
            item_feat_id['max'] = max(v)
        if min(v) < item_feat_id['min']:
            item_feat_id['min'] = min(v)
    ...
    if searcher == 'optuna':
        ...
        trials, best_params = tuner.tune(n_trials=n_trials, model_opt=model_opt, dataset=dataset, model=model,
                                         ...
                                         cuda=cuda, user_feat_id=user_feat_id, item_feat_id=item_feat_id)
        return trials, best_params

    if searcher == 'grid':
        print('######### trainer ###########')
        print('alpha1', alpha1, 'lr:', lr, 'l2:', l2, 'bs:', bsize)
        model = CausECTR(model_opt, dataset, model, optimizer, data_dir, user_feat_id, item_feat_id, bsize, alpha1, lr,
                         l2, cuda).to(device)
