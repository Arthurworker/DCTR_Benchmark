...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = DRCTR(self.model_opt, self.dataset, self.model, self.optimizer, self.data_dir,
...
class DRCTR(torch.nn.Module):
    def __init__(self, model_opt, dataset, model, optimizer, data_dir, user_feat_tab, item_feat_tab, all_user_id,
                 all_item_id, bsize, beta1=0.5, lr=1e-3, l2=1e-2, cuda=0):
        ...
        self.imputation_network.load_state_dict(torch.load(
            'pretrain/' + self.dataset + '_' + self.model + '_combine' + '.pt'))
...
def drctr(model_opt, dataset, model, data_dir, seed=0, bsize=256, beta1=0.5, lr=5e-4, l2=1e-5, n_trials=100, epoch=5000,
          cuda=0, searcher='grid', optimizer='Adam', with_test=False, **unused):
    progress = WorkSplitter()

    progress.section("DR-CTR: Set the random seed")
    setup_seed(seed)

    progress.section("DR-CTR: Estimate pscore")
    ...
    progress.section("DR-CTR: Training")
    ...
    if searcher == 'grid':
        ...
        model = DRCTR(model_opt, dataset, model, optimizer, data_dir, user_feat_tab, item_feat_tab, all_user_id,
