...
class Objective:
    ...
    def __init__(self, model_opt, dataset, model, optimizer, data_dir, seed, device, cuda, user_feat_tab, item_feat_tab,
                 all_user_id, all_item_id, epoch=500) -> None:
        """Initialize Class"""
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = BridgeCTR(self.model_opt, self.dataset, self.model, self.optimizer, self.data_dir,
...
class Tuner:
    ...
    @staticmethod
    def tune(n_trials, model_opt, dataset, model, optimizer, data_dir, seed, device, epoch, cuda, user_feat_tab,
        ...
        objective = Objective(model_opt=model_opt, dataset=dataset, model=model, optimizer=optimizer, data_dir=data_dir,
                              seed=seed, epoch=epoch, device=device, cuda=cuda, user_feat_tab=user_feat_tab,
...
class BridgeCTR(torch.nn.Module):
    def __init__(self, model_opt, dataset, model, optimizer, data_dir, user_feat_tab, item_feat_tab, all_user_id,
                 all_item_id, bsize, alpha1=0.5, lr=1e-3, l2=1e-2, cuda=0):
        ...
        self.network.load_state_dict(torch.load('pretrain/' + self.dataset + '_' + self.model + '_bias' + '.pt'))
        self.unif_network.load_state_dict(torch.load('pretrain/' + self.dataset + '_' + self.model + '_unif' + '.pt'))
...
def bridgectr(model_opt, dataset, model, data_dir, seed=0, bsize=256, alpha1=0.5, lr=5e-4, l2=1e-5, n_trials=100,
              epoch=5000, cuda=0, searcher='grid', optimizer='Adam', with_test=False, **unused):
    progress = WorkSplitter()

    progress.section("Bridge-CTR: Set the random seed")
    setup_seed(seed)

    progress.section("Bridge-CTR: Training")
    ...
    if searcher == 'optuna':
        ...
        trials, best_params = tuner.tune(n_trials=n_trials, model_opt=model_opt, dataset=dataset, model=model,
                                         ...
                                         cuda=cuda, user_feat_tab=user_feat_tab, item_feat_tab=item_feat_tab,
                                         all_user_id=all_user_id, all_item_id=all_item_id)
        return trials, best_params

    if searcher == 'grid':
        print('######### trainer ###########')
        print('alpha1', alpha1, 'lr:', lr, 'l2:', l2, 'bs:', bsize)
        model = BridgeCTR(model_opt, dataset, model, optimizer, data_dir, user_feat_tab, item_feat_tab,
                          all_user_id, all_item_id, bsize, alpha1, lr, l2, cuda).to(device)
