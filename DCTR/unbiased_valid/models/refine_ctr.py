...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = RefineCTR(self.model_opt, self.dataset, self.model, self.optimizer, self.data_dir,
...
class Tuner:
    ...
    @staticmethod
    def tune(n_trials, model_opt, dataset, model, optimizer, data_dir, seed, device, epoch, cuda):
...
class RefineCTR(torch.nn.Module):
    def __init__(self, model_opt, dataset, model, optimizer, data_dir, bsize,
                 beta1=0.01, lr=1e-3, l2=1e-5, cuda=0):
        super(RefineCTR, self).__init__()
        self.lr = lr
        self.l2 = l2
        self.bs = bsize
        self.beta1 = beta1
    ...
    def train_on_batch(self, label, data):
        ...
        unif_label = self.unif_network(data)
        unif_label = (unif_label - torch.min(unif_label)) / (torch.max(unif_label) - torch.min(unif_label))
        refine_label = torch.sigmoid(label + self.beta1 * unif_label)

        log_loss = self.criterion(logit, refine_label)
        loss = log_loss
...
def refinectr(model_opt, dataset, model, data_dir, seed=0, bsize=256, beta1=0.5, lr=5e-4, l2=1e-5, n_trials=100,
              epoch=5000, cuda=0, searcher='grid', optimizer='Adam', with_test=False, **unused):
    progress = WorkSplitter()

    progress.section("Refine-CTR: Set the random seed")
    setup_seed(seed)

    progress.section("Refine-CTR: Training")
    ...
    if searcher == 'grid':
        ...
        model = RefineCTR(model_opt, dataset, model, optimizer, data_dir, bsize, beta1, lr, l2, cuda).to(device)
