...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = TJRCTR(self.model_opt, self.dataset, self.model, self.optimizer, self.data_dir,
...
def tjrctr(model_opt, dataset, model, data_dir, seed=0, bsize=256, alpha1=0.5, alpha2=0.5, lr=5e-4, l2=1e-5,
           n_trials=150, epoch=5000, cuda=0, searcher='grid', optimizer='Adam', with_test=False, **unused):
    progress = WorkSplitter()

    progress.section("TJR-CTR: Set the random seed")
    setup_seed(seed)

    progress.section("TJR-CTR: Training")
    ...
    if searcher == 'grid':
        ...
        model = TJRCTR(model_opt, dataset, model, optimizer, data_dir, bsize, alpha1, alpha2, lr, l2, cuda).to(device)
